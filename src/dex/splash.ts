...
export class Splash extends BaseDex {
    ...
    public async liquidityPoolFromUtxo(provider: BaseDataProvider, utxo: UTxO): Promise<LiquidityPool | undefined> {
        ...
        try {
            ...
            const [lpTokenPolicyId, lpTokenAssetName] =
                typeof parameters.LpTokenPolicyId === 'string' && typeof parameters.LpTokenAssetName === 'string'
    ...
    public async buildSwapOrder(
        ...
        const swapOutToken: Token =
            swapParameters.SwapOutTokenPolicyId === 'lovelace'
                ? 'lovelace'
        ...
        const outDecimals: number =
            swapOutToken === 'lovelace'
                ? 6
                : tokensMatch(swapOutToken, liquidityPool.assetA)
                ? (liquidityPool.assetA as Asset).decimals
                : (liquidityPool.assetB as Asset).decimals;
