...
import pool from '@dex/definitions/spectrum/pool';
import order from '@dex/definitions/spectrum/order';
import { BaseApi } from '@dex/api/base-api';
import { bignumber, ConfigOptions, FormatOptions, create, all, MathJsStatic, BigNumber } from 'mathjs';
...
export class Spectrum extends BaseDex {
    ...
    async liquidityPools(provider: BaseDataProvider): Promise<LiquidityPool[]> {
        const allUtxos: UTxO[][] = await Promise.all([provider.utxos(this.poolAddress), provider.utxos(this.poolAddress2)]);
    ...
    public async liquidityPoolFromUtxo(provider: BaseDataProvider, utxo: UTxO): Promise<LiquidityPool | undefined> {
        ...
        const relevantAssets = utxo.assetBalances.filter((assetBalance) => {
            ...
            // TODO Improve method of identification here to not rely solely on an assetName.
    ...
    public async buildSwapOrder(liquidityPool: LiquidityPool, swapParameters: DatumParameters, spendUtxos: UTxO[] = []): Promise<PayToAddress[]> {
        ...
        // console.log('datum', datumBuilder);
